...
public class UserDAO {
	...
	public User checkCredentials(String username, String password) throws SQLException {
		...
		String query = "SELECT username, name, surname, mail FROM user  WHERE username = ? AND password =?";
	 ...
	 * @param username	The username to be checked
	 * @return	True if and only if the username is availbale
	 */
	public boolean isAvailableUsername(String username) throws SQLException {
		String query = "SELECT username FROM user  WHERE username = ?";
		try (PreparedStatement pstatement = connection.prepareStatement(query);){
			pstatement.setString(1, username);
			try (ResultSet result = pstatement.executeQuery();) {
	...
	public void createUser(User user) throws SQLException {
		String query = "INSERT into user (username, name, surname, mail, password) VALUES(?, ?, ?, ?, ?)";
		try (PreparedStatement pstatement = connection.prepareStatement(query);) {
